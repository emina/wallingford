...
(define (make-node circuit)
  ...
  (always* (equal? 0 (foldl + 0 (map lead-current (node-leads nd)))) #:context circuit)
  nd)
...
(define (make-battery circuit [intv null])
  ...
  (always* (equal? internal-voltage (- (node-voltage (lead-node plus)) (node-voltage (lead-node minus)))) #:context circuit)
...
(define (make-ground circuit)
  ...
  (always* (equal? 0 (node-voltage (lead-node ld))) #:context circuit)
