...
(define (make-circle context [initial-value (circle (point 150 150) 50 (color "blue"))])
  (define-symbolic* r number?)
  (define-symbolic* c color?)
  (define circ (circle (make-point) r c))
  ; give it a default value
  (assert (equal? circ initial-value))
  (stay (circle-radius circ) #:context context)
  (stay (circle-center circ) #:context context)
  (stay (circle-color circ) #:context context)
  (send context solve)
  circ)

(define (make-midpointline context)
  (define line1 (make-line))
  (define midpoint (make-point))
  (always (equal? midpoint (point-scale (point-plus (line-end1 line1) (line-end2 line1)) 0.5)) #:context context)
  (midpointline line1 midpoint))

(define (make-midpointline-with-stays context)
  (define line1 (make-line))
  (define midpoint (make-point))
  ...
  (always (equal? midpoint (point-scale (point-plus (line-end1 line1) (line-end2 line1)) 0.5)) #:context context)
  (stay (line-end1 line1) #:priority low #:context context)
  (stay (line-end2 line1) #:priority low #:context context)
